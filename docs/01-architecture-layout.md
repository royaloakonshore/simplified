...
- Key features and recent changes:
    ...
    - `InventoryItem` model: Uses `itemType: ItemType`. `quantityOnHand` is calculated. `defaultVatRatePercent` field added. **New requirements include: directly editable `quantityOnHand` field (replacing initial/adjust by X), `leadTimeDays` field, `vendorSku` and `vendorItemName` fields (conditional visibility), and linking to `InventoryCategory` for filtering.**
...
- **Inventory & Pricelist:**
    ...
    - **NEXT:** Implement a single, directly editable `quantityOnHand` field in `InventoryItemForm` and the inventory list table (with a new tRPC mutation for quick stock adjustment), replacing the previous "initial quantity/adjust by X" approach.
    - **NEXT:** Add `leadTimeDays` field to `InventoryItem` and display it.
    - **NEXT:** Add `vendorSku` and `vendorItemName` fields (conditionally hidden for manufactured goods) to `InventoryItem`.
    - **NEXT:** Add "Product Category" (`InventoryCategory`) column to the inventory table and enable filtering by it (displaying categories as pill tags).
...
1.  **Inventory Module Enhancements:** Directly editable `quantityOnHand`, `leadTimeDays` field, `vendorSku`/`vendorItemName` fields, `InventoryCategory` integration (pill tags, filtering), advanced table features.
